...
function resolveMetadata(token, record) {
    ...
    var metadata = HookMetadata.get(token);
    if (metadata) {
...
export function instantiate(token, record, ctx) {
    ...
    var instance = guardCyclicDependency(token, record, next);
...
export function executeInstantiation(token, record, ctx) {
    ...
    if (!metadata) {
        var instance_1 = record.factory(ctx, ResolveMode.Sync);
    ...
    var instance;
    var baseFactory = function () { return record.factory(ctx, ResolveMode.Sync); };
