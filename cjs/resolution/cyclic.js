...
function guardCyclicDependency(token, record, next) {
    var rec = record;
    if (rec.flags & metadata_1.RecordFlags.MaskInstantiating) {
        return handleCyclicReference(rec);
    }
    rec.flags = (rec.flags || 0) | metadata_1.RecordFlags.MaskInstantiating;
    try {
        var instance = next();
        if (rec.flags & metadata_1.RecordFlags.MaskHasProxy) {
            linkProxyState(rec, instance);
            rec.flags &= ~metadata_1.RecordFlags.MaskHasProxy;
            rec.__proxy__ = undefined;
        }
        return instance;
    }
    finally {
        rec.flags &= ~metadata_1.RecordFlags.MaskInstantiating;
    }
}
